...
class SpecterScanner:
    ...
    async def run_scan(self, config: ScanConfig) -> Any:
        engine_config = EngineConfig(
            ...
            request_timeout=config.timeout_ms / 1000,
            scan_delay=config.scan_delay,
            randomize_ports=config.randomize_ports,
            syn_scan=config.syn_scan,
            decoy_scan=config.decoy_scan,
        ...
        if result:
            ...
            svc_count = (
                sum(len(d.services) for d in result.devices)
                if result.devices and hasattr(result.devices[0], "services")
                else 0
            )
    ...
    def _setup_logging(self) -> None:
        ...
        logging.basicConfig(
            level=logging.DEBUG if self._scan_config.debug else logging.INFO,
            handlers=[handler],
