...
class NetworkMapper:
    ...
    def __init__(self, timeout: float=1.0, cache_dir: str='.specter-cache', logger: Optional[logging.Logger]=None) -> None:
        ...
        self._virtual_mac_prefixes = {'00:05:69': 'vmware', '00:0C:29': 'vmware', '00:1C:14': 'vmware', '00:50:56': 'vmware', '08:00:27': 'virtualbox', '02:42:AC': 'docker'}
...
Example:
    ...
    >>> pass"""
        ...
        return FingerprintData(ttl=ttl, window_size=window, dhcp_options=dhcp_list, user_agent=ua, smb_version=smb, ua_family=ua_family, smb_dialect=smb_dialect)
...
Example:
    ...
    >>> pass"""
        ...
        def run_probe() -> FingerprintData:
            ...
            """
            try:
                syn_pkt = IP(dst=ip) / TCP(dport=80, flags='S', options=[('MSS', 1460), ('SAckOK', b''), ('TS', (0, 0))])
                ...
                return FingerprintData(ttl=ttl, window_size=window, df_bit=df_bit, tcp_options=tcp_opts, tcp_option_order=tcp_order, icmp_types=icmp_types, icmp_codes=icmp_codes, tcp_flags=tcp_flags)
            except Exception:
                return FingerprintData()
...
Example:
    ...
    >>> pass"""
        ...
        for device in devices:
            ...
            payload.append({'ip': device.ip, 'mac': device.mac, 'hostname': device.hostname, 'os_guess': device.os_guess, 'vendor': vendor, 'device_type': device_type, 'honeypot': honeypot, 'virtualization': virtualization, 'open_ports': device.open_ports, 'services': [{'port': svc.port, 'protocol': svc.protocol, 'name': svc.service_name, 'version': svc.version} for svc in device.services]})
...
Example:
    ...
    >>> pass"""
        return {'windows': {'ttl': '128', 'window': '64240'}, 'linux': {'ttl': '64', 'window': '5840'}, 'freebsd': {'ttl': '64', 'window': '65535'}, 'cisco': {'ttl': '255', 'window': '4128'}}
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f'http://ip-api.com/json/{ip}', timeout=5) as response:
                    ...
                    if payload.get('status') != 'success':
                        return None
                    return {'country': payload.get('country', ''), 'region': payload.get('regionName', ''), 'city': payload.get('city', ''), 'isp': payload.get('isp', '')}
        except Exception:
            return None
...
Example:
    ...
    >>> pass"""
        ...
        for device in devices:
            try:
                iterator = nextCmd(SnmpEngine(), CommunityData('public', mpModel=0), UdpTransportTarget((device.ip, 161), timeout=1, retries=0), ContextData(), oids, lexicographicMode=False)
...
Example:
    ...
    >>> pass"""
        palette = {'server': '#4E9A06', 'workstation': '#3465A4', 'iot': '#F57900', 'infrastructure': '#AD7FA8', 'unknown': '#888A85'}
