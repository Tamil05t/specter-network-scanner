...
class RouterExplorer:
    ...
    def __init__(
        self,
        timeout: float = 3.0,
        router_scan_enabled: bool = False,
        max_attempts_per_vendor: int = 5,
        attempt_delay: float = 2.0,
        on_event: Optional[EventCallback] = None,
        logger: Optional[logging.Logger] = None,
        correlation_hook: Optional[Callable[[str], Awaitable[None]]] = None,
    ) -> None:
        ...
        self._vendor_paths = {
            ...
            "netgear": ["/start.htm", "/login.cgi"],
            "linksys": ["/index.htm", "/Wireless.htm"],
            "asus": ["/Main_Login.asp", "/index.asp"],
            "tenda": ["/login.asp", "/index.asp"],
            "ubiquiti": ["/login", "/api/login"],
            "mikrotik": ["/webfig"],
        }
    ...
    async def explore(
        ...
        if admin_paths:
            findings.append(
                Vulnerability(
                    cve_id="INFO",
                    description=f"Admin panel paths detected: {', '.join(admin_paths)}",
                    severity="info",
        ...
        if fingerprint.vendor:
            findings.append(
                Vulnerability(
                    cve_id="INFO",
                    description=f"Router vendor fingerprint: {fingerprint.vendor}",
                    severity="info",
                    affected_service="http",
                    exploit_db_id=None,
                )
        ...
        if self._router_scan_enabled:
            ...
            if creds:
                ...
                findings.append(
                    Vulnerability(
                        cve_id="INFO",
                        description=f"Default credentials valid for user {user}",
                        severity="high",
                        affected_service="http",
                        exploit_db_id=None,
                    )
    ...
    async def detect_admin_panels(
        self, ip: str, session: aiohttp.ClientSession, rate_limiter: RateLimiter
    ) -> List[str]:
    ...
    async def router_vuln_checks(
        ...
        if await self._check_dns_rebinding(ip, session):
            findings.append(
                Vulnerability(
                    cve_id="INFO",
                    description="DNS rebinding protection appears weak",
                    severity="medium",
                    affected_service="http",
                    exploit_db_id=None,
                )
        ...
        if await self._check_tr069(ip):
            findings.append(
                Vulnerability(
                    cve_id="INFO",
                    description="TR-069/TR-064 service exposed",
                    severity="high",
                    affected_service="cwmp",
                    exploit_db_id=None,
                )
            )
        if await self._check_snmp_write(ip):
            findings.append(
                Vulnerability(
                    cve_id="INFO",
                    description="SNMP write community likely enabled",
                    severity="high",
        ...
        if await self._check_upnp_exposed(ip, session):
            findings.append(
                Vulnerability(
                    cve_id="INFO",
                    description="UPnP exposes internal services",
                    severity="medium",
                    affected_service="upnp",
                    exploit_db_id=None,
                )
            )
        if await self._check_wps_pin(ip, session):
            findings.append(
                Vulnerability(
                    cve_id="INFO",
                    description="WPS PIN prompt detected; potential WPS PIN exposure",
                    severity="medium",
                    affected_service="http",
                    exploit_db_id=None,
                )
        ...
        if await self._check_csrf_weakness(ip, session):
            findings.append(
                Vulnerability(
                    cve_id="INFO",
                    description="Admin panel forms lack CSRF tokens",
                    severity="medium",
                    affected_service="http",
                    exploit_db_id=None,
                )
