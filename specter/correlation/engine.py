...
class ExploitCorrelator:
    ...
    def __init__(
        ...
        nvd_rate_window: int = 30,
        correlation_ttl: int = 900,
    ...
    def _heuristic_inference(self, service: Service) -> List[ExploitMatch]:
        ...
        if name in {"ftp", "redis", "mongodb", "elasticsearch"}:
            matches.append(
                ExploitMatch(
                    exploit_db_id=0,
                    title=f"Heuristic: possible unauthenticated {name} exposure",
                    cve_list=[],
                    platform=name,
                    exploit_type="remote",
                    verified=False,
                    confidence=0.6,
                    match_reason="heuristic_exposure",
                    exploit_path=None,
                )
            )
        if "default" in banner or "admin" in banner:
            matches.append(
                ExploitMatch(
                    exploit_db_id=0,
                    title="Heuristic: possible default credentials",
                    cve_list=[],
                    platform=name or "generic",
                    exploit_type="remote",
                    verified=False,
                    confidence=0.6,
                    match_reason="heuristic_default_creds",
                    exploit_path=None,
                )
            )
        return matches
    ...
    def _build_match(self, record: ExploitRecord, confidence: float, reason: str) -> ExploitMatch:
        ...
        Example:
            ...
            >>> pass"""
        return ExploitMatch(
            exploit_db_id=record.exploit_db_id,
            title=record.title,
    ...
    def _build_schema(self) -> fields.Schema:
        ...
        return fields.Schema(
            exploit_db_id=fields.ID(stored=True),
            title=fields.TEXT(stored=True),
            description=fields.TEXT(stored=True),
            platform=fields.KEYWORD(stored=True, commas=True),
            exploit_type=fields.KEYWORD(stored=True, commas=True),
            cve=fields.KEYWORD(stored=True, commas=True),
        )
